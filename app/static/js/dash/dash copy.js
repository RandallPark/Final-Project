function dashboard(id, fData) {
    ...
    function segColor(c) { return { positive: "#38A1F3", negative: "#e04014" }[c]; }

    // compute total for each age_started.
    fData.forEach(function(d) { d.total = d.positive + d.negative; });
    ...
    function pieChart(pD) {
        ...
        // var arc = d3.arc().outerRadius(pieDim.r - 10).innerRadius(0); <- for d3.v5
        var arc = d3.svg.arc().outerRadius(pieDim.r - 10).innerRadius(0);

        // create a function to compute the pie slice angles.
        // var pie = d3.pie().sort(null).value(function(d) { return d.treatment_status; }); <- d3.v5
    ...
    function histoGram(fD) {
        ...
        var x = d3.scale.ordinal().rangeRoundBands([0, hGDim.w], 0.1)
            // var x = d3.scaleBand().rangeRoundBands([0, hGDim.w], 0.1) <- d3.v5
